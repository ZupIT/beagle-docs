...
On the use example of the page creating an Android project with Beagle , you saw how to [**implement Beagle**]({{< ref path="/get-started/creating-a-project-from-scratch/case-android" lang="en" >}}) to create a simple server-driven screen with only 2 texts: 1 title and 1 paragraph.
...
This moment is where the [**BFF**]({{< ref path="/key-concepts#backend-for-frontend" lang="en" >}}) happens, it is when frontend connects to the backend.  
...
{{% alert color="info" %}}
In case there is some problem, we recommend that you follow the tutorial to [**implement the backend installation.**]({{< ref path="/get-started/installing-beagle/backend" lang="en" >}})
{{% /alert %}}
