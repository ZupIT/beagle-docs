...
`navigation` deals with the [**navigate**](/api/actions/navigate) types of action in your application. There is one implementation that it is used with default value, but that it can be replaced for a [**BeagleNavigation**](/resources/customization/beagle-for-ios/navigation) class. 
