...
You will create a button as a server-driven component, so you are able to display it on a native screen.  
...
At the end of the process, you will "call" the native screen that will shown up as in the image below.  Remember in this example, it was created in a screen composed on a `UILabel` and in a `BeagleScreenViewController` variable type, where it remains the  server-driven component.
