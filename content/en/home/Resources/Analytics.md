...
| :--------------------- | :------------------------------------------------------------- | :----------------------------------------------------------------------- |
| **getConfig**          | Method that returns an [**AnalyticsConfig**]({{<ref "#analytics-config-anchor">}}). It can also be used to initialize your analytics services when necessary                 | ``Method``           |
| **createRecord**       | This method will be called every time Beagle needs to create a record. If you don't want to send one request for each analytics generated by Beagle, it is a good idea to implement a batch mechanism within this function.                                                                              | ``Method``           |
| **getMaximumItemsInQueue**       | This method implements the size of an analytics queue. The size can be specified or not. When not specified, the maximum number of items in the queue will be 100. This is the queue to hold the records while getConfig() is not ready                                                                              | ``Method``           |
