...
| :---  |  :----  |  :---  |
| **getConfig** |  Beagle uses this configuration to know how to handle analytics events, and so this method will be called on every triggered event, in order to access the most updated global analytics config. It can return an [**AnalyticsConfig**]({{<ref "#analytics-config-anchor">}}) or `null`. When `null`, Beagle will temporarily cache records to send them when there is a new config. | ``Method`` |
...
| **getMaximumItemsInQueue**  |  Represents how many records will be temporarily stored in a queue while `getConfig()` is `null`. Defaults to `100`.  |  ``Method``  |
