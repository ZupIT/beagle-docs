...
Beagle Web offers some cache strategies, one of them is the [**Beagle's default cache protocol guided by the backed**]({{< ref path="/resources/cache#how-does-the-cache-protocol-works" lang="en" >}}). 
...
This strategy only implements [**Beagle's cache protocol**]({{< ref path="/resources/cache/" lang="en" >}}). This means that, it works the same way the standard one, but without the fallback. 
