...
Caso queira manter o estilo original de carregamento, basta executar a implementação padrão e seguir para o [**tratamento de erros**](/pt/recursos/customização/beagle-para-ios/carregamento-e-tratamento-de-erros).
...
* `lazyLoad(Request.Error)`: A requisição para carregar um [**componente Lazy**](/pt/api/componentes/lazy) falhou. Veja abaixo quando isso ocorre.
* `action(Swift.Error)`: A execução de uma [**Action**](/pt/api/ações) falhou.

**Erros em uma requisição** \(`Request.Error`\):

* `urlBuilderError`: A URL do recurso ou a [**baseURL**](/pt/recursos/customização/beagle-para-ios/dependências-do-beagle#urlbuilder) é inválida.
