...
Este handler é usado para uma ação de [**deep link navigation**](/pt/recursos/customização/beagle-para-android/deep-link-handler). A variável possui um valor default, você pode adicionar novas telas ou substituir por outras na aplicação. 
...
O `navigation` lida com tipo de ações de [**navigate**](/pt/api/ações/navigate/) da sua aplicação. Existe uma implementação que é usada como valor default, mas que também pode ser substituída por uma classe que  está de acordo com BeagleNavigation

Nesse mesmo atributo também é possível definir uma [**animação de navegação**](/pt/recursos/customização/beagle-para-ios/animações-de-navegação).

## PreFetchHelper

Este componente é usado como pré-busca do [**`BeagleScreenViewControllers`**](/pt/recursos/customização/beagle-para-ios/screen-view-controller) e entrega uma experiência para o usuário mais fluida. Ela já possui um valor e pode ser alterado para se adequar em que você precisa.
