...
- O Contexto Global é um `Objeto` do tipo [`ContextData`](/pt/home/api/context/).
...
- A partir de um componente declarativo \(utilizando a Ação [**`SetContext`**](/pt/home/api/actions/setcontext)\) ou
- Programaticamente, utilizando a função `set()` a partir do objeto `GlobalContext`

### 1. Usando o SetContext

É possível definir ou modificar um contexto global a partir da Ação [**`SetContext`**](/pt/home/api/actions/setcontext) Para tal, basta identificar o contexto global utilizando a `ID = "global"` na propriedade `contextId`, como mostrado no exemplo abaixo:
...
{{% tab name="iOS" %}}
No iOS o Contexto Global é um dos atributos do [**Beagle Dependencies**](/pt/home/resources/customization/beagle-for-ios/beagles-dependencies) .
