...
No exemplo abaixo a chamamos somente de &lt;BeagleActivity&gt;. Esse nome é você quem define ao criar sua BeagleActivity. Em nosso tutorial [**Criando um projeto do zero** ](/pt/primeiros-passos/criando-um-projeto-do-zero/case-android), chamamos ela de `AppBeagleActivity`
{{% /alert %}}
...
No entanto, esse elemento possui outros atributos, que podem ser utilizados na transição e entre telas.  Para saber mais sobre essa classe, veja em [**Screen Request**](/pt/api/screen-request) 
{{% /tab %}}

{{% tab name="iOS" %}}
Para renderizar uma tela [**server-driven**](/pt/principais-conceitos#server-driven-ui), basta criar uma instância **`BeagleScreenViewController`** do tipo **`remote`** e fazer a inicialização com a URL do seu [**BFF**](/pt/principais-conceitos#backend-for-frontend), como no exemplo abaixo:
...
Para mais detalhes sobre esse componente clique no link a seguir e verifique os detalhes do [**Remote View**](/pt/recursos/customização/beagle-para-web/parâmetros-remote-view) 
{{% /alert %}}
