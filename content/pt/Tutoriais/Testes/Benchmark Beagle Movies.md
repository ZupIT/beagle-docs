...
De modo geral, **é esperado que o payload do** [**Server-Driven UI**](/pt/principais-conceitos#server-driven-ui) **seja maior que o nativo**, pois toda estrutura de layout vem no payload, enquanto o nativo somente as informações a serem mostradas.

Apesar disso, em algumas telas o tamanho do payload é inferior ao server driven. Isso acontece porque [**BFF \(Backend For Frontend\)**](/pt/principais-conceitos#backend-for-frontend) filtra os dados que não são usados no aplicativo e não envia no payload. 
...
Para telas mais complexas, o payload do server-driven ficará maior. É importante ressaltar que o Beagle utiliza um mecanismo de [**cache**](/pt/recursos/cache/) e que esse teste é considerado apenas a primeira requisição. 
