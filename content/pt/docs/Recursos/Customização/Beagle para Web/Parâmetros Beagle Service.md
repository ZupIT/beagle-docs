...
| strategy | [**Strategy**](estrategias-de-cache) | beagle-with-fallback-to-cache |  | Estratégia de cache aplicada nas requisições de telas ao servidor |
| fetchData | fetch |  |  | Permite adicionar uma função customizada para fazer requisições HTTP. |
| analytics | [**Analytics**](../../../../api/analytics) |  |  | Permite o uso de handlers para a captura e tagueamento de alguns eventos. |
| customActions | Map&lt;string,[**ActionHandler**](acoes-customizadas)**&gt;** |  |  | Um mapa de ações customizadas que podem ser interpretadas pela biblioteca Beagle. É um mapa chave e valor onde a chave será sempre um identificador começado por `beagle:` ou `custom:` e o valor será o [**ActionHandler** ](acoes-customizadas#criando-um-actionhandler)ligado aquela ação. |
...
| useBeagleHeaders | boolean | true |  | Usar ou não [**headers específicos do Beagle**](headers-padroes) ao fazer as requisições de telas para o servidor |
