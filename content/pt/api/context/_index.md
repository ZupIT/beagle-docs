...
Perceba que o contexto foi declarado e seus valores foram definidos e usados para preencher os textos, porém é possível definir esses valores depois usando o método [`SetContext()`](/pt/api/actions/setcontext).
...
Perceba que o contexto foi declarado e seus valores foram definidos e usados para preencher os textos, porém é possível definir depois esses valores usando o método [`SetContext()`](/pt/api/actions/setcontext) . Dessa forma, você pode preencher os componentes com dados que ainda não estavam no JSON.
...
{{% alert color="info" %}}
Isso significa que esse tipo de contexto é criado por meio de [**eventos**](/pt/api/events).
