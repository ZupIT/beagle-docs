...
{{% alert color="warning" %}}
Veja como [**instalar o Beagle**](/pt/primeiros-passos/instalando-o-beagle/) caso ainda não tenha concluído esta etapa.
{{% /alert %}}
...
> Principal abordagem usada no Beagle por ser fundamentada no conceito de [**server-driven UI**](/pt/principais-conceitos#server-driven-ui). Ela permite que você construa layouts dinamicamente, consumindo as informações fornecidas por um [**BFF**](/pt/principais-conceitos#backend-for-frontend), por meio de uma URL remota.
...
{{% alert color="info" %}}
Note que a **`Screen`** está sendo construída utilizando **`Flex`**. Para entender melhor esses componentes e suas propriedades, vá para a seção [**posicionamento de componentes**](/pt/recursos/posicionamento-de-componentes/).
{{% /alert %}}
...
{{% alert color="info" %}}
Caso queira utilizar o tipo **`remote`**, veja [**como exibir uma tela Server-Driven UI**](/pt/tutoriais/exibindo-uma-tela).
{{% /alert %}}
...
👉Para entender melhor **como configurar o Beagle** e utilizá-lo aplicando o conceito de server-driven UI, veja como [**criar um projeto iOS do zero**](/pt/primeiros-passos/criando-um-projeto-do-zero/case-ios/).
