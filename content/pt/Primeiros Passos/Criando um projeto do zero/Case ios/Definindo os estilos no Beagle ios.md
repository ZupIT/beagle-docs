...
No exemplo prático da página "Criando um projeto com Beagle para iOS", você pode conferir como fazer a [**implementação do Beagle**](/pt/primeiros-passos/criando-um-projeto-do-zero/case-ios) e criar sua primeira tela server-driven.

Neste tutorial, a ideia é avançar a partir do [**projeto anterior**](/pt/primeiros-passos/criando-um-projeto-do-zero/case-ios#passo-3-configurar-o-xcode) para **criar um Design System na sua biblioteca do Beagle**.  Os exemplos que ilustrarão o passo a passo estão nestes[ **sample codes**](https://github.com/ZupIT/beagle-examples) que você encontra no nosso repositório. 
...
Essa `struct` irá conter todos os estilos que serão acessados pelo Beagle. Aqui, estará listada a lógica que permite ao Beagle aplicar os estilos definidos no iOS a uma [**tela server-driven**](/pt/principais-conceitos#server-driven-ui).
