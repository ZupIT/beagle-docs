...
One of the main advantages is that Beagles facilitates the development because of backend's configuration based on [**BFF paradigm.**](principais-conceitos.md#backend-for-frontend)
...
Yes! As a cross-platform framework, Beagle helps you on developing application on these three platforms by using the [**Server-Driven UI concept**](principais-conceitos.md#server-driven-ui), which enables that even a BFF may be used on all of these applications. 
