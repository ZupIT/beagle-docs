...
Justify-content defines the alignment of items along the main axis \(horizontally\) and has the following attributes:**`flex-start, flex-end, center, space-between, space-around e space-Evenly.`**
...
Items are aligned from the end of the main axis; aligned at  screen the bottom
...
The first item is moved to main axis start, the last is moved to the main axis end and the rest are evenly distributed making the space between them is equal:
...
Items are evenly distributed along the main axis. Equal margins are assigned at main axis start and endd \(or left and right, depending on the direction of the main axis\). For this reason, the first and last items are not sticked at the edges.
