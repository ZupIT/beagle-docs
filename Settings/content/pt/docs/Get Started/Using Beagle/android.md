...
title: "Android"
description: >
  Nesta seção, você encontra o passo a passo para fazer as configurações para o uso do Beagle em projetos Android.
---
...
{% code title="build.gradle\(Module:AppName\)" %}
```text
defaultConfig {
...
![BeagleSetup file](../../.gitbook/assets/beaglesetup.png)
...
{% code title="BeagleLoggerDefault.kt" %}
```kotlin
...
{% code title="MemoryLocalStore.kt" %}
```kotlin
...
{% code title="DatabaseLocalStore.kt" %}
```kotlin
...
{% code title="build.gradle\(Module\)" %}
```text
plugins {
